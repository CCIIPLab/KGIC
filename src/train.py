...
from model import KGIC
import logging
# from draw_simgcl import tsne_plot
logging.basicConfig(format="[%(asctime)s] %(levelname)s: %(message)s", level=logging.INFO)


def train(args, data_info):
    logging.info("================== training KGIC ====================")
    train_data = data_info[0]
    eval_data = data_info[1]
    test_data = data_info[2]
    user_triple_set = data_info[5]
    item_triple_set = data_info[6]
    user_potential_triple_set = data_info[7]
    item_origin_triple_set = data_info[8]
    # # for visualization
    # user_set = np.array([list(item) for item in user_triple_set.values()])
    # user_set = torch.LongTensor(user_set).view(-1, 1).squeeze().cuda()
    ...
    for step in range(args.n_epoch):
        ...
        while start < train_data.shape[0]:
            ...
            scores, CL_loss = model(*_get_feed_data(args, train_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set, start, start + args.batch_size))
        ...
        # tsne_plot(model.entity_emb(user_set), step)
        eval_auc, eval_f1 = ctr_eval(args, model, eval_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set)
        test_auc, test_f1 = ctr_eval(args, model, test_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set)
        ctr_info = 'epoch %.2d    eval auc: %.4f f1: %.4f    test auc: %.4f f1: %.4f'
        logging.info(ctr_info, step, eval_auc, eval_f1, test_auc, test_f1)
        if args.show_topk:
            topk_eval(args, model, train_data, test_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set)


def ctr_eval(args, model, data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set):
    auc_list = []
    f1_list = []
    model.eval()
    start = 0
    while start < data.shape[0]:
        ...
        scores, _ = model(*_get_feed_data(args, data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set, start, start + args.batch_size))
...
def topk_eval(args, model, train_data, test_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set):
    ...
    for user in user_list:
        ...
        while start + args.batch_size <= len(test_item_list):
            ...
            scores, _ = model(*_get_feed_data(args, input_data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set, 0, args.batch_size))
...
def _init_model(args, data_info):
    ...
    model = KGIC(args, n_entity, n_relation)
    if args.use_cuda:
        model.cuda()
...
def _get_feed_data(args, data, user_triple_set, item_triple_set, user_potential_triple_set, item_origin_triple_set, start, end):
    # origin item
    ...
    user_potential_triple = _get_triple_tensor(args, data[start:end, 0], user_potential_triple_set)
    item_origin_triple = _get_triple_tensor(args, data[start:end, 1], item_origin_triple_set)
    return items, users_triple, items_triple, user_potential_triple, item_origin_triple
