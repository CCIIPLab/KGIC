...
def load_data(args):
    ...
    train_data, eval_data, test_data, user_init_entity_set, item_init_entity_set, user_potential_entity_set, item_origin_entity_set = load_rating(args)
    ...
    logging.info("constructing users' potential kg triple sets ... ")
    user_potential_triple_sets = kg_propagation(args, kg, user_potential_entity_set, args.user_potential_triple_set_size, False)
    logging.info("constructing irems' origin kg triple sets ... ")
    item_origin_triple_sets = kg_propagation(args, kg, item_origin_entity_set, args.item_origin_triple_set_size, False)

    return train_data, eval_data, test_data, n_entity, n_relation, user_triple_sets, item_triple_sets, user_potential_triple_sets, item_origin_triple_sets
...
def sub_propagation(first_user, user_history_item_dict, item_history_user_dict,user_potential_item_dict):
    for first_user in user_history_item_dict.keys():
        for init_item in user_history_item_dict[first_user]:
            user_potential_item = []
            for click_user in item_history_user_dict[init_item]:
                user_potential_item = np.concatenate((user_potential_item, user_history_item_dict[click_user]))
        user_potential_item_dict[first_user] = list(set(user_potential_item))
...
def dataset_split(rating_np):
    ...
    user_init_entity_set, item_init_entity_set, user_potential_entity_set, item_origin_entity_set = collaboration_propagation(rating_np, train_indices)
    ...
    return train_data, eval_data, test_data, user_init_entity_set, item_init_entity_set, user_potential_entity_set, item_origin_entity_set
...
def collaboration_propagation(rating_np, train_indices):
    ...
    user_potential_item_dict = dict()
    item_origin_item_dict = dict()
    ...
    logging.info("construct user's potetial initial entity set ...")
    for first_user in user_history_item_dict.keys():
        for init_item in user_history_item_dict[first_user]:
            user_potential_item = []
            for click_user in item_history_user_dict[init_item]:
    ...
    logging.info("contructing items' origin entity set ...")
    for item in item_list:
        item_origin_item_dict[item] = [item]

    return user_history_item_dict, item_neighbor_item_dict, user_potential_item_dict, item_origin_item_dict
